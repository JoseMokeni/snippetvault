...
describe('Public API', () => {
  ...
  describe('GET /public/explore', () => {
    ...
    test('should return only public snippets', async () => {
      const db = getTestDb()
      await createTestSnippet(db, testUser.id, { title: 'Public Snippet', isPublic: true })
      await createTestSnippet(db, testUser.id, { title: 'Private Snippet', isPublic: false })
    ...
    test('should filter by language', async () => {
      const db = getTestDb()
      await createTestSnippet(db, testUser.id, { title: 'TS Snippet', language: 'typescript', isPublic: true })
      await createTestSnippet(db, testUser.id, { title: 'Python Snippet', language: 'python', isPublic: true })
    ...
    test('should search by title', async () => {
      const db = getTestDb()
      await createTestSnippet(db, testUser.id, { title: 'React Hook Example', isPublic: true })
      await createTestSnippet(db, testUser.id, { title: 'Docker Setup', isPublic: true })
    ...
    test('should include user info with snippets', async () => {
      const db = getTestDb()
      await createTestSnippet(db, testUser.id, { title: 'Test Snippet', isPublic: true })
    ...
    test('should return available languages', async () => {
      const db = getTestDb()
      await createTestSnippet(db, testUser.id, { language: 'typescript', isPublic: true })
      await createTestSnippet(db, testUser.id, { language: 'python', isPublic: true })
  ...
  describe('GET /public/users/:username', () => {
    test('should return user profile with public snippets', async () => {
      const db = getTestDb()
      await createTestSnippet(db, testUser.id, { title: 'Public', isPublic: true })
      await createTestSnippet(db, testUser.id, { title: 'Private', isPublic: false })
  ...
  describe('GET /public/users/search', () => {
    test('should search users by username', async () => {
      const res = await app.request(`/public/users/search?q=${testUser.username.slice(0, 4)}`)

      expect(res.status).toBe(200)
      const data = await res.json()
      expect(data.users.length).toBeGreaterThanOrEqual(1)
      expect(data.users.some((u: { username: string }) => u.username === testUser.username)).toBe(true)
    })
    ...
    test('should return public snippet count', async () => {
      ...
      expect(res.status).toBe(200)
      const data = await res.json()
      const user = data.users.find((u: { username: string }) => u.username === testUser.username)
